# This R-Code fits a hidden Markov model (HMM) to play-call data of all NFL teams and predicts play-calls of the
# 2017 season.
...
# 1. subset the corresponding data (training data: res; test data: res.17)
...
# generate training dataset for one model for all teams
res <- list()
res <- data.list.all[[1]]
length.res <- length(res)
for(i in 2:length(data.list.all)){
  res[(length.res + 1):(i*128)] <- data.list.all[[i]]
  length.res <- length(res)
}

# generate test dataset for one model for all teams
res.17 <- list()
res.17 <- data.list.17.all[[1]]
length.res <- length(res.17)
for(i in 2:length(data.list.17.all)){
  res.17[(length.res + 1):(i*16)] <- data.list.17.all[[i]]
  length.res <- length(res.17)
}
...
L.hmm <- function(theta.star, x, N){
  ...
  for(k in 1:length(x)){
    ...
    ind <- which(!is.na(x[[k]]$PlayType.new)) # oder PlayType
    ...
    ydstogo <- x[[k]]$ydstogo
    playtimediff <- x[[k]]$PlayTimeDiff
    scorediff <- x[[k]]$ScoreDiff
    nrdrive <- x[[k]]$NrDrive
    playmin1 <- c(0, x[[k]]$PlayType.new[1:(nrow(x[[k]]) - 1)])  # oder PlayType
    ...
    l <- log(sum(foo))
    phi <- foo/sum(foo)
    for(t in 2:nrow(x[[k]])){ # bis nrow(x[[k]])
...
theta.star <- c(rep(-2, 2), rep(-2.3, 2), qlogis(0.4), qlogis(0.3), qlogis(0.55), qlogis(0.53), qlogis(0.45), qlogis(0.55),
                ...
                qlogis(0.7))
L.hmm(theta.star = theta.star, res, 2)


# function for optimizing the likelihood
mle.2states <- function(theta, N){
  theta.star <- theta
  mod <- nlm(L.hmm, theta.star, x = res, N = N, print.level = 2, iterlim = 1000, hessian = TRUE) 
...
CIs <- cbind(mod$wp - 1.96 * sds, mod$wp + 1.96 * sds)
CIs_fin <- rbind(CIs[5:25, ])
round(CIs_fin, digits = 3)
...
forecast.probs <- hmm.forecast(c(0:1), res.17, mod = mod)
...
true.play <- rbindlist(res.17)$PlayType.new
...
# PCP by down
data.17 <- as.data.frame(rbindlist(res.17))
data.17$PlayType.Predict <- forecast.plays
data.17$PlayTypePredictCorrect <- ifelse(data.17$PlayType.Predict == data.17$PlayType.new, 1 , 0)
aggregate(PlayTypePredictCorrect ~ down, data = data.17, FUN = mean)

# Figure 2 ----------------------------------------------------------------
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")

data.17$ydstogov2 <- data.17$ydstogo
data.17$ydstogov2[data.17$ydstogo>=20] <- 20

p <- ggplot(data.17, aes(ydstogov2, PlayTypePredictCorrect))
p + geom_bar(stat = "summary", fun.y = "mean", aes(fill = factor(down)), position  = "dodge") +
  scale_fill_manual("down", values = cbPalette) + xlab("yards to go") + 
  ylab("correct predicted play calls") + theme_light() +
